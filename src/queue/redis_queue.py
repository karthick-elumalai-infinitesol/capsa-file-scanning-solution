...
def _is_redis_timeout(exc: Exception) -> bool:
    """Return True for redis-py socket timeout variants without hard import dependency."""
    ...
    def _connect(self):
        try:
            ...
            self._redis = rds.from_url(
                ...
                # Must be greater than the BLPOP dequeue timeout used by
                # ConcurrentScanner.scan_from_queue(timeout=20). Otherwise an
                # empty queue produces noisy socket timeout errors before BLPOP
                # can return None normally.
                socket_timeout=35,
    ...
    def dequeue(self, timeout: int = 30) -> Optional[QueueMessage]:
        ...
        try:
            ...
            if _is_redis_timeout(exc):
                # redis-py can raise timeout errors for socket reads.
                # This is expected when the worker is long-polling an empty queue;
                # treat it as no message rather than an ECS ERROR-level event.
                logger.debug("Redis dequeue timed out waiting for messages: %s", exc)
                return None
